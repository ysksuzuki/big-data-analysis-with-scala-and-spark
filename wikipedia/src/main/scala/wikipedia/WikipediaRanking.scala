...
object WikipediaRanking {
  ...
  val conf: SparkConf = new SparkConf().setAppName("wikipedia-article").setMaster("local")
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = rdd.aggregate(0)(
    (acc, w) => {
      if (w.mentionsLanguage(lang)) acc + 1
      else acc
    },
    (acc1, acc2) => acc1 + acc2
  )
  ...
  private def findLangs(langs: List[String], article: WikipediaArticle) = langs.filter(article.mentionsLanguage)
